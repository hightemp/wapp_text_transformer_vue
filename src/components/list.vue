...
export default {
    ...
    computed: {
        sSelectedID: {
            get() {
                return this.$store.state.oDatabase[this.table_name][`selection_index`]
            },
            set(mV) {
                this.$store.commit('fnUpdateDatabaseVar', { sTableName: this.table_name, sVarName: `selection_index`, mV })
            }
        },
        oSelectedItem: {
            get() {
